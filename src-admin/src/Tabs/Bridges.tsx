...
export class Bridges extends BridgesAndDevices<BridgesProps, BridgesState> {
    ...
    renderBridgeEditDialog() {
        ...
        const save = () => {
            ...
            this.setState({ editBridgeDialog: null }, () =>
                this.props.updateConfig(matter));
        };

        const isDisabled = this.state.editBridgeDialog.name === this.state.editBridgeDialog.originalName &&
        ...
        >
            ...
            <DialogContent>
                ...
                {isCommissioned
                    ? I18n.t(
                        'Bridge is already commissioned. You cannot change the name or the vendor/product ID.',
                    )
                    : null}
            </DialogContent>
            <DialogActions>
    ...
    renderDeviceEditDialog() {
      ...
      !!this.props.commissioning[
          this.props.matter.bridges[this.state.editDeviceDialog.bridgeIndex].uuid
      ];
        ...
        const isDisabled =
      (this.state.editDeviceDialog.name ===
        this.state.editDeviceDialog.originalName &&
        this.state.editDeviceDialog.deviceType ===
          this.state.editDeviceDialog.originalDeviceType &&
        this.state.editDeviceDialog.dimmerOnLevel ===
          this.state.editDeviceDialog.originalDimmerOnLevel &&
        this.state.editDeviceDialog.dimmerUseLastLevelForOn ===
          this.state.editDeviceDialog.originalDimmerUseLastLevelForOn &&
        this.state.editDeviceDialog.actionAllowedByIdentify ===
          this.state.editDeviceDialog.originalActionAllowedByIdentify &&
        this.state.editDeviceDialog.noComposed ===
          this.state.editDeviceDialog.originalNoComposed) ||
      (!this.state.editDeviceDialog.dimmerUseLastLevelForOn &&
        !this.state.editDeviceDialog.dimmerOnLevel) ||
      !this.state.editDeviceDialog.deviceType;
    ...
    renderDeleteDialog() {
        ...
        if (
            this.state.suppressDeleteTime &&
      this.state.suppressDeleteTime > Date.now()
        ) {
            setTimeout(() => {
                if (!this.state.deleteDialog) {
                    return;
                        ...
                        onClick={() => {
                            ...
                            if (this.state.deleteDialog) {
                                if (this.state.deleteDialog.type === 'bridge') {
                                    ...
                                    matter.bridges[
                                        this.state.deleteDialog.bridgeIndex
                                    ].list.splice(this.state.deleteDialog.deviceIndex || 0, 1);
                                }
                            }
                            this.setState(
                                {
    ...
    renderAddDeviceDialog() {
        ...
        return (
            <DeviceDialog
                ...
                addDevices={(devices: DetectedDevice[]) => this.addDevicesToBridge(
                    devices,
                    this.state.addDeviceDialog?.bridgeIndex || 0,
                    true,
                )}
                ...
                type="device"
                name={
                    this.props.matter.bridges[this.state.addDeviceDialog.bridgeIndex].name
                }
            />
        );
    }
    ...
    renderDevice(
        ...
        return (
            ...
            >
                <TableCell
                    style={{ border: 0, borderBottomLeftRadius: isLast ? 4 : 0 }}
                />
                <TableCell>
                ...
                <TableCell
                    style={{ width: 0, borderBottomRightRadius: isLast ? 4 : 0 }}
                >
                    <Tooltip
            ...
            >
                <TableCell
                    style={{
                        ...
                        borderBottomLeftRadius: this.state.bridgesOpened[bridgeIndex]
                            ? 0
                            : 4,
                    }}
                    sx={styles.bridgeButtonsAndTitle}
                ...
                <TableCell
                    style={{
                        ...
                        borderBottomRightRadius: this.state.bridgesOpened[bridgeIndex]
                            ? 0
                            : 4,
                    }}
                    sx={styles.bridgeButtonsAndTitle}
            ...
            {this.state.bridgesOpened[bridgeIndex] ? <>
                <TableRow>
                    ...
                    <TableCell
                        ...
                        sx={styles.devicesHeader}
                    >
                        {this.props.alive && bridge.enabled && this.props.nodeStates[bridge.uuid]?.status === 'waitingForCommissioning' ? <Tooltip
                                ...
                                onClick={() => {
                                    this.props.socket
                                        ...
                                        .then(result => {
                                            if (result.error) {
                                                window.alert(
                                                    `Cannot re-announce: ${result.error}`,
                                                );
                                            } else {
                    ...
                    <TableCell
                        ...
                        sx={styles.devicesHeader}
                    >
                        {this.props.alive && bridge.enabled ? <Tooltip
                        ...
                        >
                            <IconButton
                                onClick={async () => {
                                    ...
                                    if (!isLicenseOk) {
                                        this.props.alive && this.props.showToast(I18n.t(
    ...
    render() {
        return <div>
            ...
            >
                <Fab
                    ...
                    onClick={async () => {
                        ...
                        if (!isLicenseOk) {
                            this.props.alive && this.props.showToast(
            ...
            {this.props.matter.bridges.length ? <div>
                ...
                >
                    <span>
                        <IconButton
                            ...
                            disabled={Object.values(this.state.bridgesOpened).every(
                                v => v === true,
                            )}
                        >
                            <UnfoldMore />
                ...
                'No bridges created. Create one, by clicking on the "+" button in the bottom right corner.',
            )}
            <Table
                size="small"
            ...
            >
                <TableBody>
                    {this.props.matter.bridges.map((bridge, bridgeIndex) =>
                        this.renderBridge(bridge, bridgeIndex))}
                </TableBody>
            </Table>
        </div>;
    }
