...
import { I18n, type ThemeType, InfoBox } from '@iobroker/gui-components';
...
export default class QrCodeDialog extends Component<QrCodeDialogProps, QrCodeDialogState> {
    ...
    constructor(props: QrCodeDialogProps) {
        ...
        const wifiList = QrCodeDialog.selectableCredentials(config?.additionalWifiCredentials || []);
        const threadList = QrCodeDialog.selectableCredentials(config?.additionalThreadCredentials || []);
        this.state = {
            manualCode: '',
            qrCode: '',
            qrError: '',
            hideQrCode: false,
            iframe: null,
            // Pre-select the first named set when the default set is empty (only additional ones configured).
            wifiCredentialId:
                QrCodeDialog.isDefaultWifiSet(config) || !wifiList.length ? DEFAULT_CREDENTIAL_ID : wifiList[0].id,
            threadCredentialId:
                QrCodeDialog.isDefaultThreadSet(config) || !threadList.length
                    ? DEFAULT_CREDENTIAL_ID
                    : threadList[0].id,
        };
    }

    /** The default (scalar) WiFi set is usable only when both SSID and password are present. */
    private static isDefaultWifiSet(config?: MatterControllerConfig): boolean {
        return !!(config?.wifiSSID && config.wifiPassword);
    }

    /** The default (scalar) Thread set is usable only when a network name and a dataset are present. */
    private static isDefaultThreadSet(config?: MatterControllerConfig): boolean {
        return !!(config?.threadNetworkName && config.threadOperationalDataSet);
    }
            ...
            <div style={{ marginTop: 16, maxWidth: 400 }}>
                ...
                {wifi.length ? (
                    ...
                    >
                        ...
                        >
                            {QrCodeDialog.isDefaultWifiSet(config) ? (
                ...
                {thread.length ? (
                    ...
                    >
                        ...
                        >
                            {QrCodeDialog.isDefaultThreadSet(config) ? (
                                <MenuItem value={DEFAULT_CREDENTIAL_ID}>{I18n.t('Default')}</MenuItem>
                            ) : null}
                            {thread.map(entry => (
                                <MenuItem
