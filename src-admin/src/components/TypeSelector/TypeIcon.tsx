...
export default function TypeIcon(props: TypeIconProps): React.JSX.Element | null {
    ...
    if (props.src) {
        return (
            <Icon
                style={defaultStyle}
                {...props}
                title={props.title === true ? I18n.t(`type-${props.type}`) : props.title}
                src={props.src}
            />
        );
    }

    const TypeIcon = props.type && TYPE_ICONS[props.type];
    if (!TypeIcon) {
        // Show the first letter of a type
        return props.type ? (
            <span style={{ ...defaultStyle, ...(props.style || undefined) }}>{props.type[0].toUpperCase()}</span>
        ) : null;
    }
